...
namespace base
{
    namespace
    {
        ...
        // inside the version string and constructs a vector of valid integers.
        // It stops when it reaches an invalid item (including the wildcard
        // character). |parsed| is the resulting integer vector. Function
        // returns true if all numbers were parsed successfully, false
        // otherwise.
                                 ...
                                 std::vector<uint32_t>* parsed)
        {
            ...
            for (auto item : numbers)
            {
                ...
                unsigned int num;
                if (!StringToInt(item, &num)) return false;

                // This throws out leading zeros for the first item only.
                if (item == *numbers.begin() && std::to_string(num) != item)
                    return false;

                // StringToUint returns unsigned int but Version fields are
                // uint32_t.
        ...
        // |components2|. Returns -1, 0 or 1 if |components1| is less than,
        // equal to, or greater than |components2|, respectively.
        int CompareVersionComponents(const std::vector<uint32_t>& components1,
                                     const std::vector<uint32_t>& components2)
        {
            const size_t count =
                std::min(components1.size(), components2.size());
        ...
        const std::string& wildcard_string) const
    {
        ...
        // If the version is smaller than the wildcard version's |parsed|
        // vector, then the wildcard has no effect (e.g. comparing 1.2.3
        // and 1.3.*) and the version is still smaller. Same logic for equality
        // (e.g. comparing 1.2.2 to 1.2.2.* is 0 regardless of the wildcard).
        // Under this logic, 1.2.0.0.0.0 compared to 1.2.* is 0.
        if (comparison == -1 || comparison == 0) return comparison;

        // Catch the case where the digits of |parsed| are found in
        // |components_|, which means that the two are equal since |parsed| has
        // a trailing "*". (e.g. 1.2.3 vs. 1.2.* will return 0). All other cases
        // return 1 since components is greater (e.g. 3.2.3 vs 1.*).
        const size_t min_num_comp = std::min(components_.size(), parsed.size());
        for (size_t i = 0; i < min_num_comp; ++i)
        {
            if (components_[i] != parsed[i]) return 1;
        }
        return 0;
    }
