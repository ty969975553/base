...
namespace Test
{
    ...
    void UnitTest::PrintMessage(const std::wstring& string, MessageType kind)
    {
        ...
        switch (kind)
        {
            ...
            case MessageType::File:
                SetColor(true, false, true, true);
                break;
            case MessageType::Category:
                SetColor(true, true, false, true);
                break;
...
#endif
    {
        if (argc < 3)
        {
            ...
            {
                PrintMessage(L"Failures are not suppressed.",
                             MessageType::Info);
            }
            ...
            PrintMessage(L"Passed test files: " + itow(passedFiles) + L"/" +
                             itow(totalFiles),
                         MessageType);
            PrintMessage(L"Passed test cases: " + itow(passedCases) + L"/" +
                             itow(totalCases),
                         MessageType);
            testContext = nullptr;
            return passed ? 0 : 1;
    ...
    void UnitTest::RegisterTestFile(const char* fileName,
    ...
    void UnitTest::RunCategoryOrCase(const std::wstring& description,
                                     bool isCategory, std::function<void()>&& callback)
    {
        if (!testContext)
            throw UnitTestConfigError(
                L"TEST_CATEGORY is not allowed to execute outside of "
                L"TEST_FILE.");
        if (testContext->kind == UnitTestContextKind::Case)
            throw UnitTestConfigError(
                isCategory
                    ? L"TEST_CATEGORY is not allowed to execute inside "
                      L"TEST_CASE"
                    : L"TEST_CASE is not allowed to execute inside TEST_CASE");

        PrintMessage(description,
                     (isCategory ? MessageType::Category : MessageType::Case));

        UnitTestContext context;
        context.parent = testContext;
        context.indentation = testContext->indentation + L"    ";
        context.kind = isCategory ? UnitTestContextKind::Category
                                  : UnitTestContextKind::Case;
