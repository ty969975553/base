//
#pragma once
#include "base/serialization/ptree_fwd.hpp"
#include "base/template/define.hpp"
#include "base/serialization/detail/ptree_utils.hpp"
#include <utility>
#include <optional>
...
namespace base
{
    namespace property_tree
    {
        ...
        public:
            ...
            // until the container is complete. Sucks. Especially for the
            // reverses.
            class iterator;
            ...
            /** Insert a copy of the given tree with its key just before the
             * given position in this node. This operation invalidates no
             * iterators.
             ...
             * assoc_iterator.
             * @return A valid iterator pointing to the element after the
             * erased.
             */
            ...
            /** Sorts the direct children of this node according to the
             * predicate. The predicate is passed the whole pair of key and
             * child.
             */
            ...
            /** Two property trees are the same if they have the same data, the
             * keys and order of their children are the same, and the children
            ...
            /** Find a child with the given key, or not_found() if there is
             * none. There is no guarantee about which child is returned if
             * multiple have the same key.
             */
            ...
            std::pair<assoc_iterator, assoc_iterator> equal_range(
                const key_type &key);

            /** Find the range of children that have the given key. */
            std::pair<const_assoc_iterator, const_assoc_iterator> equal_range(
                const key_type &key) const;

            /** Count the number of direct children with the given key. */
            size_type count(const key_type &key) const;

            /** Erase all direct children with the given key and return the
             * count.
             */
            size_type erase(const key_type &key);

            /** Get the iterator that points to the same element as the
             * argument.
            ...
            /** Get the iterator that points to the same element as the
             * argument.
             * @note A valid const_assoc_iterator range (a, b) does not imply
             * that (to_iterator(a), to_iterator(b)) is a valid range.
             */
             ...
             *       completely deterministic, i.e. if the same key appears
             * multiple times, which child is chosen is not specified. This can
             * lead to the path not being resolved even though there is a
             *       descendant with this path. Example:
             * @code
             *   a -> b -> c
             *     -> b
             * @endcode
             *       The path "a.b.c" will succeed if the resolution of "b"
             * chooses the first such node, but fail if it chooses the second.
             */
            ...
            std::optional<self_type &> get_child_optional(const path_type &path);

            /** Get the child at the given path, or return boost::null. */
            std::optional<const self_type &> get_child_optional(
                const path_type &path) const;

            /** Set the node at the given path to the given value. Create any
             * missing parents. If the node at the path already exists, replace
             * it.
             * @return A reference to the inserted subtree.
             * @note Because of the way paths work, it is not generally
             * guaranteed that a node newly created can be accessed using the
             * same path.
             * @note If the path could refer to multiple nodes, it is
             * unspecified which one gets replaced.
             */
            self_type &put_child(const path_type &path, const self_type &value);

            /** Add the node at the given path. Create any missing parents. If
             * there already is a node at the path, add another one with the
             * same key.
            ...
            typename std::enable_if<detail::is_translator<Translator>,
                                      Type>::type
            ...
            /** Take the value of this node and attempt to translate it to a
             * @c Type object using the supplied translator. Return @p
             * default_value if this fails.
             */
            template <class Type, class Translator>
            ...
            typename  std::enable_if<th::is_character<Ch>,
                                      std::basic_string<Ch> >::type
            get_value(const Ch *default_value, Translator tr) const;

            /** Take the value of this node and attempt to translate it to a
             * @c Type object using the default translator. Return @p
             * default_value if this fails.
             */
            template <class Type>
            typename th::disable_if<detail::is_translator<Type>, Type>::type
            ...
            /** Take the value of this node and attempt to translate it to a
             * @c Type object using the supplied translator. Return boost::null
             * if this fails.
             */
            template <class Type, class Translator>
            ...
            /** Take the value of this node and attempt to translate it to a
             * @c Type object using the default translator. Return boost::null
             * if this fails.
             */
            template <class Type>
             ...
             * return boost::null;
             * @endcode
             * That is, return the value if it exists and can be converted, or
             * nil.
             */
            template <class Type, class Translator>
            optional<Type> get_optional(const path_type &path,
                                        Translator tr) const;
            ...
            /** Set the value of the node at the given path to the supplied
             * value, translated to the tree's data type. If the node doesn't
             * exist, it is created, including all its missing parents.
        ...
        private:
            ...
            // Getter tree-walk. Not const-safe! Gets the node the path refers
            // to, or null. Destroys p's value.
