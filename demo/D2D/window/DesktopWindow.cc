...
// </summary>
HRESULT DesktopWindow::Initialize(_In_ RECT bounds, _In_ std::wstring title)
{
...
// </summary>
void DesktopWindow::SetNewDpi(_In_ float newPerMonitorDpi)
{
...
HRESULT DesktopWindow::Render()
{
    ...
    {
        hr = m_deviceResources->GetDxgiFactory()->RegisterOcclusionStatusWindow(
            m_hWnd, WM_USER, &m_occlusion);
        if (FAILED(hr))
        {
            return hr;
        }
...
// </summary>
LRESULT DesktopWindow::OnCreate(_In_ UINT, _In_ WPARAM, _In_ LPARAM lParam,
                                _Out_ BOOL &bHandled)
{
    ...
    UINT dpix;
    UINT dpiy;
    // if (FAILED(GetDpiForMonitor(monitor, MONITOR_DPI_TYPE::MDT_EFFECTIVE_DPI,
    // &dpix, &dpiy)))
    //{
    dpix = 96;
    dpiy = 96;
...
//
LRESULT DesktopWindow::OnDestroy(_In_ UINT, _In_ WPARAM, _In_ LPARAM,
                                 _Out_ BOOL &bHandled)
{
    m_visible = false;
...
LRESULT DesktopWindow::OnPaint(_In_ UINT, _In_ WPARAM, _In_ LPARAM,
                               _Out_ BOOL &bHandled)
{
    HDC hDC;
    PAINTSTRUCT ps;
...
LRESULT DesktopWindow::OnWindowPosChanged(_In_ UINT, _In_ WPARAM,
                                          _In_ LPARAM lparam,
                                          _Out_ BOOL &bHandled)
{
    RECT clientRect;
    ...
    if (!(windowPos->flags & SWP_NOSIZE))
    {
        ...
        size.Width = static_cast<float>(clientRect.right - clientRect.left) /
                     (m_deviceResources->GetDpi() / 96.0F);
        size.Height = static_cast<float>(clientRect.bottom - clientRect.top) /
                      (m_deviceResources->GetDpi() / 96.0F);
...
LRESULT DesktopWindow::OnDisplayChange(_In_ UINT, _In_ WPARAM, _In_ LPARAM,
                                       _Out_ BOOL &bHandled)
{
    Render();
    OnDisplayChange();
...
LRESULT DesktopWindow::OnGetMinMaxInfo(_In_ UINT, _In_ WPARAM,
                                       _In_ LPARAM lparam, _Out_ BOOL &bHandled)
{
...
LRESULT DesktopWindow::OnActivate(_In_ UINT, _In_ WPARAM wparam, _In_ LPARAM,
                                  _Out_ BOOL &bHandled)
{
...
LRESULT DesktopWindow::OnPowerBroadcast(_In_ UINT, _In_ WPARAM,
                                        _In_ LPARAM lparam,
                                        _Out_ BOOL &bHandled)
{
    if (lparam > 0)
...
LRESULT DesktopWindow::OnOcclusion(_In_ UINT, _In_ WPARAM, _In_ LPARAM,
                                   _Out_ BOOL &bHandled)
{
    ASSERT(m_occlusion);

    if (S_OK ==
        m_deviceResources->GetSwapChain()->Present(0, DXGI_PRESENT_TEST))
    {
        m_deviceResources->GetDxgiFactory()->UnregisterOcclusionStatus(
            m_occlusion);
        m_occlusion = 0;
        m_visible = true;
    }
...
LRESULT DesktopWindow::OnPointerDown(_In_ UINT, _In_ WPARAM, _In_ LPARAM lparam,
                                     _Out_ BOOL &bHandled)
{
    ...
    auto localx =
        static_cast<float>(pt.x) / (m_deviceResources->GetDpi() / 96.0F);
    auto localy =
        static_cast<float>(pt.y) / (m_deviceResources->GetDpi() / 96.0F);
...
LRESULT DesktopWindow::OnPointerUp(_In_ UINT, _In_ WPARAM, _In_ LPARAM lparam,
                                   _Out_ BOOL &bHandled)
{
    auto x = GET_X_LPARAM(lparam);
    auto y = GET_Y_LPARAM(lparam);
...
LRESULT DesktopWindow::OnEnterSizeMove(_In_ UINT, _In_ WPARAM, _In_ LPARAM,
                                       _Out_ BOOL &bHandled)
{
...
LRESULT DesktopWindow::OnExitSizeMove(_In_ UINT, _In_ WPARAM, _In_ LPARAM,
                                      _Out_ BOOL &bHandled)
{
...
LRESULT DesktopWindow::OnDpiChange(_In_ UINT, _In_ WPARAM wparam,
                                   _In_ LPARAM lparam, _Out_ BOOL &bHandled)
{
...
LRESULT DesktopWindow::OnPointerUpdate(_In_ UINT, _In_ WPARAM,
                                       _In_ LPARAM lparam, _Out_ BOOL &bHandled)
{
    auto x = GET_X_LPARAM(lparam);
    auto y = GET_Y_LPARAM(lparam);
